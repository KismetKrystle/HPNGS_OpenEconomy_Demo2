...
type Screen = 'landing' | 'camera' | 'home' | 'upload' | 'inbox' | 'events' | 'event-profile';

import { PublicEvent, MediaItem, Artist, EventSignup } from './types/events';
...
export default function App() {
  ...
  // Handle event signup from EventProfileScreen
  const handleEventSignup = (
    eventId: string,
    signupData: Omit<EventSignup, 'id' | 'signupDate' | 'status'>
  ) => {
    ...
    setPublicEvents(prev =>
      prev.map(ev =>
        ev.id === eventId
          ? { ...ev, signups: [...(ev.signups ?? []), newSignup] }
          : ev
      )
    );

    // Keep the selected event profile in sync if it's the same event
    setSelectedEventForProfile(prev =>
      prev && prev.id === eventId
        ? { ...prev, signups: [...(prev.signups ?? []), newSignup] }
        : prev
    );
  };


  // Minimal UI: show Events list with header
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-10 border-b bg-white/60 backdrop-blur supports-[backdrop-filter]:bg-white/40 dark:bg-background">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 text-lg font-semibold">
            <Calendar className="size-5" />
            <span>HPGNS Events</span>
          </div>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <button className="px-3 py-1.5 rounded-md border" onClick={() => setCurrentScreen('events')}>Events</button>
            <button className="px-3 py-1.5 rounded-md border" onClick={() => setCurrentScreen('upload')}>Upload</button>
            <button className="px-3 py-1.5 rounded-md border" onClick={() => setCurrentScreen('inbox')}>Inbox</button>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        {currentScreen === 'landing' ? (
          <LandingScreen onEnterSite={() => setCurrentScreen('home')} />
        ) : currentScreen === 'events' ? (
          <EventScreen
            events={publicEvents}
            artists={artists}
            onEventSelect={(ev) => {
              setSelectedEventForProfile(ev);
              setCurrentScreen('event-profile');
            }}
          />
        ) : currentScreen === 'upload' ? (
          <UploadScreen onUpload={addMediaItem} />
        ) : currentScreen === 'inbox' ? (
          <InboxScreen mediaItems={mediaItems} onUpdateStatus={updateMediaStatus} onMintNFT={mintNFT} />
        ) : currentScreen === 'event-profile' && selectedEventForProfile ? (
          <EventProfileScreen
