...
import { PublicEvent, Artist, EventAnalytics, EventSignup, MediaItem } from '../types/events';
...
export function EventScreen({ events, artists, onEventSelect }: EventScreenProps) {
  ...
  const sortedEvents = [...filteredEvents].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'popular':
        return (b.analytics?.totalLikes ?? 0) - (a.analytics?.totalLikes ?? 0);
      case 'upcoming':
        const now = new Date();
    ...
    >
      <CardContent className="p-0">
        <div className={`flex ${isCompact ? 'flex-col' : ''}`}>
          {/* Event Image - Made larger to show more of the image */}
          <div className={`${isCompact ? 'w-full h-40' : 'w-32 h-32'} bg-muted relative overflow-hidden`}>
            <img
              src={event.thumbnail}
              alt={event.name}
          ...
          {/* Event Details - Simplified to show only title, metrics, and category */}
          <div className={`flex-1 p-4 ${isCompact ? 'min-h-0' : ''}`}>
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0">
                <h3 className="font-medium mb-1 line-clamp-2">{event.name}</h3>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                <div className="flex items-center space-x-1">
                  ...
                  <span>{event.analytics?.attendeesCount ?? 0}</span>
                </div>
                <div className="flex items-center space-x-1">
                  <TrendingUp className="h-4 w-4" />
                  <span>{event.analytics?.totalLikes ?? 0}</span>
                </div>
